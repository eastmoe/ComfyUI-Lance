...
from common.inference_utils import map_splits_to_samples, make_packed_vit_token_embed, uncond_split_pro
...
class Lance(PreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.config = config
        self.inference_args: InferenceArguments = kwargs.get("inference_args") or InferenceArguments()

        if self.inference_args.use_task_embedding:
        ...
        if self.inference_args.use_modality_embedding:
    ...
    def forward(
        ...
        apply_qwen_2_5_vl_pos_emb = getattr(self.inference_args, "apply_qwen_2_5_vl_pos_emb", False)
        ...
        if self.config.visual_gen:
            ...
            if getattr(self.inference_args, "incre_time_pro", 0) <=0:
    ...
    def generate_visual(
        ...
        **kwargs,
        ...
        for i_sample in range(num_samples):
            ...
            # NOTE: 固定种子
            noise_seed = kwargs.get("noise_seed", -1)
            if noise_seed > 0:
                generator = torch.Generator(device=device).manual_seed(noise_seed + get_global_rank() * max_samples + i_sample)
            else:
                generator = None
    ...
    def understand_visual_kvcache(
        ...
        for i_sample in range(L):
            ...
            generation_input, kv_lens, ropes = self.prepare_vit_images_inference(curr_kvlens=[0], curr_rope=[0], vit_tokens = [current_val_packed_vit_tokens], new_token_ids=new_token_ids, device=device)
            past_key_values = self.forward_cache_update_vit_inference(past_key_values, vit_vae_video_grid_thw=current_val_vit_video_grid_thw, device=device, dtype=dtype,**generation_input)
    ...
    def prepare_vit_images_inference(self, curr_kvlens, curr_rope, vit_tokens, new_token_ids, device):
    ...
    @torch.no_grad()
    def forward_cache_update_vit_inference(
        self,
    ...
    # ------------------------------------------------------------------------------------------------------------------------------
    # for KV-cache visual generation
