...
def _match_tensor(target: torch.Tensor, source: torch.Tensor) -> torch.Tensor:
    if source.dtype == target.dtype and source.device == target.device:
        return source
...
class PackedAttention(Qwen2Attention):
    ...
    def forward(self, *args, **kwargs):
        if self.training or kwargs.get("mode_forward") == "inference":
...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        packed_query_states[packed_und_token_indexes] = _match_tensor(packed_query_states, self.q_proj(packed_sequence_und))
        packed_query_states[packed_gen_token_indexes] = _match_tensor(packed_query_states, self.q_proj_moe_gen(packed_sequence_gen))

        packed_key_states[packed_und_token_indexes] = _match_tensor(packed_key_states, self.k_proj(packed_sequence_und))
        packed_key_states[packed_gen_token_indexes] = _match_tensor(packed_key_states, self.k_proj_moe_gen(packed_sequence_gen))

        packed_value_states[packed_und_token_indexes] = _match_tensor(packed_value_states, self.v_proj(packed_sequence_und))
        packed_value_states[packed_gen_token_indexes] = _match_tensor(packed_value_states, self.v_proj_moe_gen(packed_sequence_gen))
        ...
        packed_query_states_[packed_und_token_indexes] = _match_tensor(packed_query_states_, self.q_norm(packed_query_states[packed_und_token_indexes]))
        if self.config.freeze_und:
            packed_query_states_[packed_und_token_indexes] = packed_query_states_[packed_und_token_indexes].detach()
        packed_query_states_[packed_gen_token_indexes] = _match_tensor(packed_query_states_, self.q_norm_moe_gen(packed_query_states[packed_gen_token_indexes]))

        packed_key_states_[packed_und_token_indexes] = _match_tensor(packed_key_states_, self.k_norm(packed_key_states[packed_und_token_indexes]))
        if self.config.freeze_und:
            packed_key_states_[packed_und_token_indexes] = packed_key_states_[packed_und_token_indexes].detach()
        packed_key_states_[packed_gen_token_indexes] = _match_tensor(packed_key_states_, self.k_norm_moe_gen(packed_key_states[packed_gen_token_indexes]))
        ...
        if isinstance(attention_mask, List):
            ...
            packed_attn_output_[packed_und_token_indexes] = _match_tensor(packed_attn_output_, self.o_proj(packed_attn_output[packed_und_token_indexes]))
            packed_attn_output_[packed_gen_token_indexes] = _match_tensor(packed_attn_output_, self.o_proj_moe_gen(packed_attn_output[packed_gen_token_indexes]))
        else:  # USED !!!
            pad_size = sum(sample_lens) - packed_query_states.shape[0]
    ...
    def forward_inference(
        ...
        **kwargs
        ...
        if mode == "und":
            ...
            packed_query_states[packed_text_indexes] = _match_tensor(packed_query_states, self.q_proj(packed_text_query_sequence))
            packed_query_states[packed_vae_token_indexes] = _match_tensor(packed_query_states, self.q_proj_moe_gen(packed_vae_query_sequence))

            packed_key_states[packed_text_indexes] = _match_tensor(packed_key_states, self.k_proj(packed_text_query_sequence))
            packed_key_states[packed_vae_token_indexes] = _match_tensor(packed_key_states, self.k_proj_moe_gen(packed_vae_query_sequence))

            packed_value_states[packed_text_indexes] = _match_tensor(packed_value_states, self.v_proj(packed_text_query_sequence))
            packed_value_states[packed_vae_token_indexes] = _match_tensor(packed_value_states, self.v_proj_moe_gen(packed_vae_query_sequence))
            ...
            packed_query_states[packed_text_indexes] = _match_tensor(packed_query_states, self.q_norm(packed_query_states[packed_text_indexes]))
            packed_query_states[packed_vae_token_indexes] = _match_tensor(packed_query_states, self.q_norm_moe_gen(packed_query_states[packed_vae_token_indexes]))

            packed_key_states = packed_key_states.to(torch.float32)
            packed_key_states[packed_text_indexes] = _match_tensor(packed_key_states, self.k_norm(packed_key_states[packed_text_indexes]))
            packed_key_states[packed_vae_token_indexes] = _match_tensor(packed_key_states, self.k_norm_moe_gen(packed_key_states[packed_vae_token_indexes]))
        ...
        if mode == "und":
            ...
            packed_attn_output[packed_text_indexes] = _match_tensor(packed_attn_output, self.o_proj(packed_attn_output[packed_text_indexes]))
            packed_attn_output[packed_vae_token_indexes] = _match_tensor(packed_attn_output, self.o_proj_moe_gen(packed_attn_output[packed_vae_token_indexes]))
...
class Qwen2MoTDecoderLayer(nn.Module):
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        packed_sequence_[packed_und_token_indexes] = _match_tensor(packed_sequence_, self.input_layernorm(packed_sequence[packed_und_token_indexes]))
        packed_sequence_[packed_gen_token_indexes] = _match_tensor(packed_sequence_, self.input_layernorm_moe_gen(packed_sequence[packed_gen_token_indexes]))
        ...
        residual = packed_sequence
        packed_sequence_ = packed_sequence.new_zeros(packed_sequence.shape)
        packed_sequence_[packed_und_token_indexes] = _match_tensor(
            packed_sequence_,
        ...
        packed_sequence_[packed_gen_token_indexes] = _match_tensor(
            packed_sequence_,
    ...
    def forward_inference(
        ...
        **kwargs
        ...
        if mode == "und":
            ...
            packed_query_sequence_[packed_text_indexes] = _match_tensor(
                packed_query_sequence_,
                self.input_layernorm(packed_query_sequence[packed_text_indexes]),
            )
            packed_query_sequence_[packed_vae_token_indexes] = _match_tensor(
                packed_query_sequence_,
        ...
        if mode == "und":
            ...
            packed_query_sequence_[packed_text_indexes] = _match_tensor(packed_query_sequence_, self.mlp(packed_text_query_sequence))
            packed_query_sequence_[packed_vae_token_indexes] = _match_tensor(packed_query_sequence_, self.mlp_moe_gen(packed_vae_query_sequence))
            packed_query_sequence = packed_query_sequence_
...
class Qwen2MoEDecoderLayer(nn.Module):
    ...
    def forward_train(
        ...
        packed_sequence_new[packed_und_token_indexes] = _match_tensor(packed_sequence_new, packed_sequence_und)
        packed_sequence_new[packed_gen_token_indexes] = _match_tensor(packed_sequence_new, packed_sequence_gen)
